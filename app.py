...
def load_models():
    ...
    try:
        ...
        st.sidebar.success("✅ Classical model loaded", icon="🤖")
    except FileNotFoundError:
        st.error("Model files not found! Please run the training script first to generate `models/best_classical_model.pkl` and `models/tfidf_vectorizer.pkl`.")
        st.stop()
    ...
    for model_name in model_names:
        try:
            ...
            st.sidebar.success(f"✅ {model_name.split('/')[-1]} loaded", icon="🤖")
        except Exception as e:
            st.sidebar.error(f"⚠️ Could not load {model_name}: {e}")

    return classical_model, tfidf_vectorizer, transformer_pipelines
...
def main():
    ...
    # Load models (cached)
    classical_model, tfidf_vectorizer, transformer_pipelines = load_models()
    if not all([classical_model, tfidf_vectorizer, transformer_pipelines]):
        st.stop() # Stop if models failed to load

    # --- Sidebar for User Input ---
    st.sidebar.header("Analysis Configuration")
    analysis_type = st.sidebar.radio("Analyze a:", ('Profile', 'Hashtag'))
    ...
    max_tweets = st.sidebar.slider("Maximum number of tweets to fetch", 10, 200, 50)
    days_ago = st.sidebar.slider("Tweets from how many days ago?", 1, 30, 7)
    
    analyze_button = st.sidebar.button("🔍 Analyze Sentiment", type="primary")

    if analyze_button:
        if not query_input:
            st.sidebar.error("Please enter a username or hashtag.")
            return
